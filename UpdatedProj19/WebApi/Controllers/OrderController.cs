...
namespace WebApi.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<IActionResult> PostOrderModel(CartOrder cartOrder)
        {
            ...
            CartMobiles cart = _context.Carts.Where(w=>w.CartID==CartId).FirstOrDefault();
            MobileModel mobile = _context.Mobiles.Where(w=>w.MobileId==cart.MobileID).FirstOrDefault();
            ...
            if(order.OrderedQuantity>mobile.MobileQuantityAvailable)
            {
                return BadRequest("Out Of Stock Or Quantity Not Available");
            }
            _context.OrderModels.Add(order);
            mobile.MobileQuantityAvailable = mobile.MobileQuantityAvailable - cart.Quantity;
            _context.Carts.Remove(cart);
            await _context.SaveChangesAsync();
            EmailDTO request = new EmailDTO();
            request.To = "amina74@ethereal.email";
            request.Subject = "Order SuccessFully Placed";
            request.Body = "Hello Dear Customer Your Order With OrderId-" + order.OrderId.ToString() + " of Mobile named " + mobile.MobileName + " has been placed Successfully.1" ;
        ...
        public async Task<IActionResult> DeleteOrderModel(int id)
        {
            ...
            var mobile = _context.Mobiles.Where(x => x.MobileName == orderModel.MobileName).FirstOrDefault();
            mobile.MobileQuantityAvailable = quantity + mobile.MobileQuantityAvailable;
            await _context.SaveChangesAsync();
